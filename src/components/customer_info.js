...
class CustomerInfo extends Component {
        ...
        <div key={index}>
          <div className="row #b0bec5 blue-grey lighten-3">
            <div className="col s12">
              <button
                disabled={preventButtonClick}
                className="small btn waves-effect  waves offset-s1 col s3 center #a5d6a7 green lighten-3"
                onClick={() => this.handleSubmit(restaurantName, ID, phone)}
              >
                notify
              </button>
              <button
                className="small btn orange waves-effect  waves offset-s1 col s3 center"
                onClick={() => this.handleSeated(ID)}
              >
                seat
              </button>
              <button
                className="small btn red waves-effect  waves offset-s1 col s3 center"
  ...
  render() {
    return (
      <Fragment>
        <div className="container">{this.renderCustomerListOnDom()}</div>
      </Fragment>
    );
  }
}
