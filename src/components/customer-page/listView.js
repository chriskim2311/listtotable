...
class ListView extends Component {
  ...
  async componentDidUpdate(prevProps) {
    ...
    if (
      (!prevData && data) ||
      (prevData && data && prevData.length !== data.length)
    ) {
  ...
  async buildRestaurantsList(results) {
    ...
    const list = await Promise.all(
      results.map(async (current, index) => {
                ...
                <div className="restaurantInfoContainer">
                  <div className="restaurantInfo">
                    <div className="starsDistanceInfo">
                      <div className="stars">
                        <span className="boldText">Stars: </span> {rating}
                      </div>
                      <div className="distance">
                        <span className="boldText">Distance: </span>
                        {distance}{" "}
                      </div>
                    </div>
                    <div className="icons">
                      <div className="dollarSignsContainer">
                        <div className="dollarSigns">{price}</div>
                      </div>
                      <Link
                        to={`/reservation-info/${name}/${current.place_id}`}
                        className="addButton"
                      >
                        <img src={addButton} />
                      </Link>
                  ...
                  <div className="address">
                    <span className="boldText">Address: </span> {address}
                  </div>
                </div>
              </div>
