...
import { busyHoursUrl } from "../../config";

export async function getRestaurantData(places, latLng, location) {
  const resp = await axios.post(`${busyHoursUrl}`, { place_id: places });
  var date = new Date();
  var day = date.getDay();
  ...
  var nav = function(input, path) {
    ...
    for (var i = 0; i < path.length; i++) {
      ...
      if (typeof curr != "object") {
        if (i == path.length - 1) return curr;
        return null;
      }
    }
    return curr;
  ...
  restaurantData.busyHour =
    nav(resp, "data", "data", "week", day, "hours", time, "percentage") ||
    restaurantData.busyHour;
